...
IN_DATA = {
    'name': 'ilforno',
    'host': 'https://delivery.ilfornogroup.ru/',
    'target_url': 'https://delivery.ilfornogroup.ru/menu/ilforno',
    'qty_items': 10,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'page'))
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[@lang_load="true"]'))
            product_block = driver.find_element(By.CLASS_NAME, 'product_detail')
            item_title = product_block.find_element(By.CLASS_NAME, 'name').text
            # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
            item_brand = IN_DATA['name']
            item_price = product_block.find_element(By.XPATH, '//div[@class="form-button"]//div').get_attribute('innerHTML')
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = ''
            WebDriverWait(product_block, 20).until(lambda d: d.find_element(By.ID, 'productImage'))
            images = product_block.find_elements(By.XPATH, '//div[@id="productImage"]//img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'productsSlider'))
        elements = driver.find_elements(By.CLASS_NAME, 'image_block')
        items = []
