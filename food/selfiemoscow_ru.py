...
IN_DATA = {
    'name': 'selfie',
    'host': 'https://delivery.selfiemoscow.ru/',
    'target_url': 'https://delivery.selfiemoscow.ru/',
    'qty_items': 10,
}
...
def get_data(driver) -> list:
    items = driver.find_elements(By.XPATH, '//a[contains(@class,"js-product-link")]/parent::div')
    items_list = []
    for item in items[:IN_DATA['qty_items']]:
        ...
        WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'js-product-name'))
        item_title = item.find_element(By.CLASS_NAME, 'js-product-name').text
        # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
        item_brand = IN_DATA['name']
        item_price = item.find_element(By.CLASS_NAME, 'js-product-price').text
        item_price = re.sub(r"[^\d\.]", "", item_price)
        item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}".encode("utf-8")).hexdigest()
        item_sizes = ''
        item_desc = item.find_element(By.CLASS_NAME, 't-descr').get_attribute('innerHTML') \
            .replace('\r', '').replace('\n', '')
        print()
        WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'js-product-img'))
        images = item.find_elements(By.CLASS_NAME, 'js-product-img')
        k = 0
        images_urls = []
        for image in images:
            images_urls.append(image.get_attribute('data-original'))
        item_images_arr = []
