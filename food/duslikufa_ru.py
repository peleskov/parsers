...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CSS_SELECTOR, '[data-product-uid]'))
            product_block = driver.find_element(By.CSS_SELECTOR, '[data-product-uid]')
            item_title = product_block.find_element(By.CLASS_NAME, 't-store__prod-popup__name').text
            item_brand = IN_DATA['name']
            item_price = product_block.find_element(By.CSS_SELECTOR, '[data-product-price-def]').get_attribute('data-product-price-def')
            ...
            item_sizes = ''
            item_desc = product_block.find_element(By.CLASS_NAME, 'js-store-prod-all-text').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
            images_urls = [product_block.find_element(By.CSS_SELECTOR, '[itemprop="image"]').get_attribute('content')]
            k = 0
            item_images_arr = []
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'store__card__wrap_txt-and-opts'))
        elements = driver.find_elements(By.CLASS_NAME, 'store__card__wrap_txt-and-opts')
        items = []
