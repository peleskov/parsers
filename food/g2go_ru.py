...
IN_DATA = {
    'name': 'Gallery to go',
    'host': 'https://www.g2go.ru/',
    'target_url': 'https://www.g2go.ru/popular',
    'qty_items': 10,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            product_block = driver.find_element(By.XPATH, '//div[contains(@class, "image")]/parent::div/parent::div')
            item_title = product_block.find_element(By.TAG_NAME, 'h1').text
            # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
            item_brand = IN_DATA['name']
            item_price = product_block.find_element(By.XPATH, '//div[contains(@class, "--cost")]').text
            item_price = re.sub(r"[^\d\.]", "", item_price).rstrip('.')
            ...
            #     .replace('\r', '').replace('\n', '')
            WebDriverWait(product_block, 20).until(lambda d: d.find_element(By.TAG_NAME, 'img'))
            images = product_block.find_elements(By.XPATH, '//img[contains(@class,"img_product")]')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'prod-block'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}'
        elements = driver.find_elements(By.CLASS_NAME, 'prod-block')
        items = []
