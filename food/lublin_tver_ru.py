...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            page_n += 1
            data = get_ids(driver, url, page_n)
            if data['items']:
                items_list.extend(data['items'])
            url = data['target_url']
            try:
                WebDriverWait(driver, 60).until(lambda d: d.find_element(By.XPATH, '//li[@class="bx-pag-next"]/a'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        item_data = {}
        try:
            # получаем каждый товар и собираем данные
            try:
                item_data_req = requests.post(IN_DATA['data_url'], data={'ID': item[0]},  headers=HEADERS)
                item_data = item_data_req.json()
            except Exception as ex:
                pass
            ...
            k = 0
            images_urls = [f'{IN_DATA["host"][:-1]}{item_data["IMAGE"]}']
            item_images_arr = []
...
def get_ids(driver, page_url, n) -> dict:
    out_data = {'items': None, 'target_url': None}
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'menu-wrapper_content'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}'
        elements = driver.find_elements(By.CLASS_NAME, 'card')
        items = []
        for element in elements:
            onclick = element.find_element(By.CLASS_NAME, 'card-info__title').get_attribute('onclick')
            items.append([
                onclick.replace('openPopupProduct(', '').replace(')', ''),
            ])
