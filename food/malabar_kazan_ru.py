...
IN_DATA = {
    'name': 'malabar-kazan.ru',
    'host': 'http://malabar-kazan.ru/',
    'target_url': 'http://malabar-kazan.ru/menu',
    'qty_items': 1000,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'off-canvas-wrapper'))
        ...
        driver.get(url)
        items_list = driver.find_elements(By.CLASS_NAME, 'food-plask')
        # Соберем данные
...
def get_data(items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        ...
        item_price = item.find_element(By.CLASS_NAME, 'price').get_attribute('innerHTML')
        item_price = re.sub(r"[^\d\.]", "", item_price)
        item_sizes = ''
        item_desc = item.find_element(By.CLASS_NAME, 'text').get_attribute('innerHTML') \
            .replace('\r', '').replace('\n', '')
        image_el = item.value_of_css_property("background-image")
        images_urls = [re.split('[()]', image_el)[1].replace('"', '')]
