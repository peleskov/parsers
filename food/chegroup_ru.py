...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'hero'))
        ...
        WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'product'))
...
def get_data(items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_sizes = ''
            item_desc = item.get_attribute('data-description') \
                .replace('\r', '').replace('\n', '')
            img = item.get_attribute('data-src').replace("url('/", "").replace("')", "")
