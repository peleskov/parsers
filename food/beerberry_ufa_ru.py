...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'menu'))
...
def get_links(driver, page_url) -> dict:
    out_data = {'items': None, 'target_url': None}
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'recomendation-item'))
        elements = driver.find_elements(By.XPATH, '//div[@class="recomendation-item"]/a')
        items = []
