...
IN_DATA = {
    'name': 'indokitay.su',
    'host': 'https://indokitay.su/',
    'target_url': 'https://indokitay.su/menyu.html',
    'qty_items': 5000,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'banner_category'))
        ...
        items_list = driver.find_elements(By.XPATH, '//ul//li/form')
        # Соберем данные
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_title = item.find_element(By.CLASS_NAME, 'name_tovar').text
            item_brand = IN_DATA['name']
            item_price = item.find_element(By.CLASS_NAME, 'tovar_price').text
            item_price = re.sub(r"[^\d\.]", "", item_price).rstrip('.')
            item_sizes = ''
            item_desc = item.find_element(By.CLASS_NAME, 'opus_tovar').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
