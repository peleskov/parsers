...
IN_DATA = {
    'name': 'baranjar.ru',
    'host': 'https://baranjar.ru/',
    'target_url': 'https://baranjar.ru/categories/Sety/',
    'qty_items': 5,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'root'))
        ...
        WebDriverWait(driver, 10).until(lambda d: d.find_element(By.TAG_NAME, 'h3'))
        items_list = driver.find_elements(By.XPATH, '//h3/parent::a/parent::div')
...
def get_data(items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_title = item.find_element(By.TAG_NAME, 'h3').text.replace('"', '').capitalize()
            # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
            item_brand = IN_DATA['name']
            item_price = item.find_element(By.CLASS_NAME, 'flex').find_element(By.TAG_NAME, 'div').text
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = item.find_element(By.TAG_NAME, 'p').text \
                .replace('\r', '').replace('\n', '<br>')

            images = [item.find_element(By.TAG_NAME, 'img')]
            k = 0
            images_urls = []
            for image in images:
