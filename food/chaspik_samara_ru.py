...
IN_DATA = {
    'name': 'chaspik-samara.ru',
    'host': 'https://chaspik-samara.ru/',
    'target_url': 'https://chaspik-samara.ru/catalog/rolls/',
    'qty_items': 5,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'index-page'))
        ...
        while True:
            descr_block = driver.find_element(By.CLASS_NAME, 'description-category')
            try:
                WebDriverWait(driver, 2).until(lambda d: d.find_element(By.CLASS_NAME, 'ajax-pager-wrap'))
                driver.execute_script("arguments[0].scrollIntoView();", descr_block)
                time.sleep(random.randint(1, 5))
                continue
            except Exception as ex:
                break
        items_list = driver.find_elements(By.CLASS_NAME, 'product')
        # Соберем данные
...
def get_data(items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_title = item.find_element(By.CLASS_NAME, 'product-title').find_element(By.TAG_NAME, 'span').text.capitalize()
            # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
            item_brand = IN_DATA['name']
            item_price = item.find_element(By.CSS_SELECTOR, '[itemprop="price"]').text
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = item.find_element(By.CSS_SELECTOR, '[itemprop="description"]').text \
                .replace('\r', '').replace('\n', '<br>')

            images = [item.find_element(By.TAG_NAME, 'source')]
            k = 0
            images_urls = []
            for image in images:
                images_urls.append(f'{IN_DATA["host"]}{image.get_attribute("srcset")}')
            item_images_arr = []
