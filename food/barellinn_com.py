...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'body_wrap'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 60).until(lambda d: d.find_element(By.CLASS_NAME, 'nextpostslink'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        ...
        if item['img']:
            ...
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'wp-post-image'))
...
def get_links(driver, page_url, n) -> dict:
    ...
    try:
        ...
        elements = driver.find_elements(By.CLASS_NAME, 'post_item_wrap')
        items = []
        for element in elements:
            h3 = element.find_element(By.TAG_NAME, 'h3')
            link = h3.find_element(By.TAG_NAME, 'a').get_attribute('href')
            title = h3.text
            desc = h3.find_element(By.XPATH, 'following::span[1]').text
            img = element.find_element(By.CLASS_NAME, "wp-post-image").get_attribute('src') not in IN_DATA['not_image']
            pw = element.find_element(By.CLASS_NAME, 'price').text.split('/')
            price = re.sub(r"[^\d\.]", "", pw[1])
            items.append({
                'link': link,
                'title': title,
                'desc': desc,
