...
IN_DATA = {
    'name': 'myasoroob.ru',
    'host': 'https://myasoroob.ru/',
    'target_url': 'https://myasoroob.ru/',
    'qty_items': 5000,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'scrollingElement'))
...
def get_data(driver) -> list:
    ...
    json_data = json.loads(driver.find_element(By.XPATH, '//script[@id="__NEXT_DATA__"]').get_attribute('innerHTML'))
    dishes = json_data['props']['initialState']['meals']['_list']['items']
    items_list = []
    for item in dishes[:IN_DATA['qty_items']]:
        # получаем каждую старницу и собираем данные
        # 'id;Title;Brand;Price;Sizes;Description;Images;\n'
        item_title = item['name']
        item_brand = IN_DATA['name']
        item_price = item['price']
        item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item['code']}".encode("utf-8")).hexdigest()
        item_sizes = ''
