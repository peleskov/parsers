...
IN_DATA = {
    'name': 'rest-pashtet.ru',
    'host': 'https://rest-pashtet.ru/',
    'target_url': 'https://rest-pashtet.ru/catalog/pashtetyi-i-zakuski',
    'qty_items': 5,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'main-slider'))
...
def get_data(items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        ...
        item_title = item.find_element(By.CLASS_NAME, 'name').find_element(By.CLASS_NAME, 'name').get_attribute('innerHTML')
        ...
        item_desc = item.find_element(By.CLASS_NAME, 'description').get_attribute('innerHTML') \
            .replace('\r', '').replace('\n', '')
        image_el = item.find_element(By.CLASS_NAME, 'image').value_of_css_property("background-image")
