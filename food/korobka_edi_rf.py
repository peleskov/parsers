...
IN_DATA = {
    'name': 'korobka_edi',
    'host': 'https://xn--80abkdwb0aar0j.xn--p1ai/',
    'target_url': 'https://xn--80abkdwb0aar0j.xn--p1ai/gastroboxes/',
    'qty_items': 10,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            images = driver.find_elements(By.CLASS_NAME, 'js-product-img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    footer_block = driver.find_element(By.ID, 't-footer')
    driver.execute_script("arguments[0].scrollIntoView();", footer_block)
    try:
