...
IN_DATA = {
    'name': 'itle.pro',
    'host': 'https://itle.pro/',
    'target_url': 'https://itle.pro/delivery',
    'qty_items': 5000,
}
...
def get_items():
    ...
    options = Options()
    # Не работает с закрытым окном браузера !!!!!!
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'contents'))
...
def get_data(driver) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        ...
        item_title = item.find_element(By.TAG_NAME, 'h6').find_element(By.TAG_NAME, 'a').get_attribute('innerHTML')
        item_brand = IN_DATA['name']
        item_price = item.find_element(By.CSS_SELECTOR, '[name="sum"]').get_attribute('value')
        item_price = re.sub(r"[^\d\.]", "", item_price)
        item_sizes = ''
        item_desc = item.find_element(By.CLASS_NAME, 'menu-description').get_attribute('innerHTML') \
            .replace('\r', '').replace('\t', '').replace('\n', '')
        item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item_desc}".encode("utf-8")).hexdigest()

        images_urls = [item.find_element(By.CSS_SELECTOR, '[data-large-image]').get_attribute('data-large-image')]
        k = 0
        item_images_arr = []
