...
IN_DATA = {
    'name': 'delivery.koreana-spb.ru',
    'host': 'https://delivery.koreana-spb.ru/',
    'target_url': 'https://delivery.koreana-spb.ru/catalog/salads/',
    'qty_items': 10,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'product-banner'))
            ...
            item_price = driver.find_element(By.CLASS_NAME, 'product-banner__buy-new--price').text
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = driver.find_element(By.CLASS_NAME, 'product__description').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'product-banner'))
            images = driver.find_elements(By.CLASS_NAME, 'product-banner')
            k = 0
            images_urls = []
            for image in images:
                images_urls.append(f'{IN_DATA["host"][:-1]}{image.get_attribute("data-image")}')
            item_images_arr = []
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'product-card'))
        elements = driver.find_elements(By.CLASS_NAME, 'product-card')
        items = []
        for element in elements:
            items.append([
                f'{IN_DATA["host"][:-1]}{element.get_attribute("data-url")}',
            ])
