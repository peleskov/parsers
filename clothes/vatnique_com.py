...
IN_DATA = {
    'name': 'vatnique.com',
    'host': 'https://vatnique.com/',
    'target_url': 'https://vatnique.com/',
    'qty_items': 1,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_price = driver.find_element(By.XPATH, '//div[contains(@class,"js-store-product")]//div[contains(@class,"js-product-price")]').get_attribute('data-product-price-def')
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            sizes = driver.find_elements(By.XPATH, '//div[contains(@class,"js-store-product")]//span[contains(@class, "t-product__option-title")]')
            sizes_list = []
            for size in sizes:
                sizes_list.append(size.text)
            item_sizes = '||'.join(sizes_list)
            item_desc1 = driver.find_element(By.XPATH, '//div[contains(@class,"js-store-product")]//div[contains(@class, "js-store-prod-all-text")]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
            item_desc2 = driver.find_element(By.XPATH, '//div[contains(@class,"js-store-product")]//div[contains(@class, "js-store-prod-all-charcs")]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
            item_desc = f'<div>{item_desc1}</div><div>{item_desc2}</div>'
            images = driver.find_elements(By.XPATH, '//div[@class="t-container"]//div[contains(@class, "t-slds__bgimg")]')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
                item_image_url = image.get_attribute("data-original")
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//div[@data-screen-min="980px"]//div[contains(@class, "t-store__card")]/a'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//div[@data-screen-min="980px"]//div[contains(@class, "t-store__card")]/a')
        items = []
