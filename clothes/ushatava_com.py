...
IN_DATA = {
    'name': 'ushatava.com',
    'host': 'https://www.ushatava.com/',
    'target_url': 'https://www.ushatava.com/catalog/women/',
    'qty_items': 18,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'main-container'))
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 5).until(lambda d: d.find_element(By.CLASS_NAME, 'container_big'))
            item_title = driver.find_element(By.XPATH, '//div[contains(@class, "card__title")]').text
            # item_brand = driver.find_element(By.XPATH, '//meta[@itemprop="brand"]').get_attribute('content')
            item_brand = IN_DATA['name']
            item_price = driver.find_element(By.XPATH, '//div[contains(@class,"card__price")]/span').text
            item_price = re.sub(r'[^\d,.]', '', item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            sizes = driver.find_elements(By.XPATH, '//div[@class="card-size"]//span[@class="checkbox__text"]')
            sizes_list = []
            for size in sizes:
                sizes_list.append(size.get_attribute('innerHTML'))
            item_sizes = '||'.join(sizes_list)
            item_desc = ''
            description = driver.find_elements(By.XPATH, '//div[contains(@class,"accordion__submenu")]/div[@class="section__text_normal"]')
            item_desc = description[0].get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
            images = driver.find_elements(By.XPATH, '//img[contains(@class,"slider-single__img-src")]')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
                item_image_url = image.get_attribute("data-origin")
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    # Проскролим страницу до появления последнего товара, если товаров не хватает
    nn = 0
    while True:
        nn += 1
        els = driver.find_elements(By.XPATH, '//div[contains(@class,"catalog-list-main")]//div[contains(@class, "catalog-element")]')
        if len(els) >= IN_DATA['qty_items']:
            break
        index = 8*nn if 8*nn < len(els) else len(els) - 1
        driver.execute_script("arguments[0].scrollIntoView();", els[index])
        try:
            WebDriverWait(driver, 2).until(lambda d: d.find_element(By.CLASS_NAME, 'js-last-page'))
        except Exception as ex:
            continue
        break
    # Соберем ссылки
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'container_big'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "catalog-element")]/a')
        items = []
