...
IN_DATA = {
    'name': 'kruzhok.store',
    'host': 'https://kruzhok.store/',
    'target_url': 'https://kruzhok.store/men/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'header2'))
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.ID, 'catalogItem2'))
            ...
            item_price = driver.find_element(By.ID, 'eCommerceItemPrice').get_attribute('value')
            ...
            images = driver.find_elements(By.CLASS_NAME, 'photo')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'catalogList2'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//p[@class="name"]/a')
        items = []
