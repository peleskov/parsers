...
IN_DATA = {
    'name': 'latrika.com',
    'host': 'https://www.latrika.com/',
    'target_url': 'https://www.latrika.com/collection/new',
    'qty_items': 1,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.CLASS_NAME, 'product-form'))
            ...
            item_price = driver.find_element(By.CLASS_NAME, 'product__price-cur').text \
                .replace('₽', '').replace(' ', '')
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            sizes = driver.find_elements(By.XPATH, '//form[@class="product-form"]//div[@class="option-values"]//button')
            sizes_list = []
            for size in sizes:
                sizes_list.append(size.text)
            item_sizes = '||'.join(sizes_list)
            item_desc = driver.find_element(By.CLASS_NAME, 'product-info').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//div[@class="js-product-all-images "]//img')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
                item_image_url = image.get_attribute("data-src")
...
def get_links(driver, page_url, n) -> dict:
    ...
    try:
        ...
        elements = driver.find_elements(By.XPATH, '//div[@class="product-preview__title"]/a')
        items = []
