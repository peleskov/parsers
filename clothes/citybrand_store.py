...
IN_DATA = {
    'name': 'citybrand.store',
    'host': 'https://ru.citybrand.store/',
    'target_url': 'https://ru.citybrand.store/shop/zhenschinam-platya',
    'qty_items': 1
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'main-page'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//ul[contains(@class, "pagination")]//a[contains(@class, "page-next")]'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_brand = driver.find_element(By.XPATH, '//a[@class="brand-name"]').text
            ...
            item_desc = driver.find_element(By.XPATH, '//ul[@class="VendorInfo__features"]/li').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
            item_sizes = '||'.join([s.text for s in driver.find_elements(By.XPATH, '//span[contains(text(), "Размер:")]/..//div')])

            images = driver.find_elements(By.XPATH, '//div[contains(@class, "product-card")]//picture//img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'catalog'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//a[contains(@class, "catalog-item-image")]')
        items = []
