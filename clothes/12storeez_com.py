...
IN_DATA = {
    'name': '12storeez.com',
    'host': 'https://12storeez.com/',
    'target_url': 'https://12storeez.com/catalog',
    'qty_items': 21,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'site-wrapper'))
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.CLASS_NAME, 'card__holder'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_info = json.loads(driver.find_element(By.XPATH, '//script[@type="application/ld+json"]').get_attribute('innerHTML'))
            item_brand = item_info[0]['brand']['name']
            item_price = item_info[0]['offers']['price']
            ...
            item_desc = driver.find_element(By.CLASS_NAME, 'card-features__text').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = item_info[0]['image']
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
                item_image_url = image['contentUrl']
...
def get_links(driver, page_url, n) -> dict:
    ...
    try:
        ...
        if req.url != page_url:
            ...
            # WebDriverWait(driver, 30).until(lambda d: d.find_element(By.TAG_NAME, 'main'))
            out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
            elements = driver.find_elements(By.CLASS_NAME, 'ProductCardSlider')
            items = []
