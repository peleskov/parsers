...
IN_DATA = {
    'name': 'iamstudio.ru',
    'host': 'https://iamstudio.ru/',
    'target_url': 'https://iamstudio.ru/catalog/',
    'qty_items': 20,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 5).until(lambda d: d.find_element(By.CLASS_NAME, 'detail_element_box'))
            ...
            sizes = driver.find_elements(By.XPATH, '//div[@class="chose-sz1"]//a')
            sizes_list = []
            for size in sizes:
                sizes_list.append(size.get_attribute('innerHTML'))
            item_sizes = '||'.join(sizes_list)
            item_desc = ''
            description = driver.find_elements(By.XPATH, '//div[@class="_body"]//div[@class="detail_element_box-material"]')
            for desc in description:
                d = desc.get_attribute('innerHTML').replace('\r', '').replace('\n', '')
                item_desc = f'{item_desc}<div>{d}</div>'
            images = driver.find_elements(By.XPATH, '//img[@class="_big_pic"]')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//section[contains(@class, "catalog_box")]'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.CLASS_NAME, 'item_detail_link')
        items = []
