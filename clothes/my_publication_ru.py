...
IN_DATA = {
    'name': 'my-publication.ru',
    'host': 'http://my-publication.ru/',
    'target_url': 'http://my-publication.ru/freelance/seo-copywriting/',
    'qty_items': 2,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        with open(path_results, 'w', newline="", encoding='UTF8') as f:
            f.write('id;Title;Master;Price;Info;Image\n')
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.CLASS_NAME, 'private-info'))
            item_title = driver.find_element(By.XPATH, '//div[@class="usermoreinfo"]/div').text
            item_master = driver.find_element(By.XPATH, '//div[@class="name"]/span').text
            item_id = hashlib.sha256(f"{item_title}{item_master}{item[0]}".encode("utf-8")).hexdigest()
            item_price = 0
            for price in item[1]:
                if price:
                    item_price = price
                    break
            item_info = driver.find_element(By.CLASS_NAME, 'content-room-info').get_attribute('innerHTML')\
                .replace('\r', '').replace('\n', '')
            item_image = ''
            item_image_url = driver.find_element(By.XPATH, '//div[@class="userpic"]//img').get_attribute('src')
            ...
            items_list.append((
                item_id,
                item_title.replace('Специализация: ', ''),
                item_master,
                item_price,
                item_info,
                item_image,
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'copyrlist'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?p={n}'
        elements = driver.find_elements(By.CLASS_NAME, 'grad')
        items = []
        for element in elements:
            price = element.find_elements(By.TAG_NAME, 'td')
            link = element.find_element(By.TAG_NAME, 'h1').find_element(By.TAG_NAME, 'a')
            items.append([
                link.get_attribute("href"),
                [p.text for p in price[4:]],
            ])
