...
IN_DATA = {
    'name': 'cocos-moscow.ru',
    'host': 'https://cocos-moscow.ru/',
    'target_url': 'https://cocos-moscow.ru/catalog/',
    'qty_items': 21,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "main_page")]'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 5).until(lambda d: d.find_element(By.CLASS_NAME, 'bx_pagination_page_list_num'))
            pages = driver.find_elements(By.XPATH, '//ul[@class="bx_pagination_page_list_num"]//li')
            page_max = int(pages[-1].text)
        except Exception as ex:
            pass
        while len(items_list) < IN_DATA['qty_items'] and url and page_n <= page_max:
            page_n += 1
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 5).until(lambda d: d.find_element(By.CLASS_NAME, 'product_card_detail'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = IN_DATA['name']
            item_price = driver.find_elements(By.XPATH, '//meta[@itemprop="price"]').get_attribute('content')
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            # sizes = driver.find_elements(By.CLASS_NAME, 'sizes-selector__size')
            # sizes_list = []
            # for size in sizes:
            #     sizes_list.append(size.get_attribute('innerHTML'))
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@class="more-info_block descr"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_element(By.XPATH, '//div[contains(@class,"product-item-detail-gallery-image")]//a')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'product__list'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}&SIZEN_1=18'
        elements = driver.find_elements(By.CLASS_NAME, 'product-item-image-wrapper')
        items = []
