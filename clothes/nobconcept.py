...
IN_DATA = {
    'name': 'nobconcept.com',
    'host': 'https://nobconcept.com/',
    'target_url': 'https://nobconcept.com/catalog/',
    'qty_items': 1,
}
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.CLASS_NAME, 'block-product__main'))
            item_title = driver.find_element(By.XPATH, '//h1[@class="block-product__title"]').text
            item_brand = driver.find_element(By.XPATH, '//p[contains(@class, "block-product__subtitle")]').text
            item_price = driver.find_element(By.XPATH, '//div[@class="block__inner"]//a[contains(@class, "i-add-to-favorite")]').get_attribute('data-price')
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            sizes = driver.find_elements(By.XPATH, '//div[contains(@class, "custom-select_size")]//button[@class="custom-select__option"]')
            sizes_list = []
            for size in sizes:
                sizes_list.append(size.get_attribute('innerHTML'))
            item_sizes = '||'.join(sizes_list)

            item_desc = driver.find_element(By.CLASS_NAME, 'block-product__desc').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//figure//img')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'product__content'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}&SIZEN_1=30'
        elements = driver.find_elements(By.CLASS_NAME, 'product__link')
        items = []
