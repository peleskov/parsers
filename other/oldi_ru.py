...
IN_DATA = {
    'name': 'oldi_ru',
    'folder': 'chehli',
    'host': 'https://www.oldi.ru/',
    'target_url': 'https://www.oldi.ru/catalog/6535/',
    'qty_items': 1000,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'mask'))
...
def get_data(driver, items, path_results) -> bool:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'prod-tr'))
            item_title = item['title']
            item_price = driver.find_element(By.CLASS_NAME, 'price_leg').text
            item_price = round(float(re.sub(r"[^\d\.]", "", item_price)))
            item_brand = driver.find_element(By.XPATH, '//div[@itemprop="brand"]//meta[@itemprop="name"]').get_attribute('content')

            item_params = ''
            try:
                flag_try = 1
                WebDriverWait(driver, 60).until(lambda d: d.find_element(By.ID, 'description'))
                prop_rows = driver.find_elements(By.XPATH, '//div[@id="description"]//table[@class="characts-list"]//tr')
                props = []
                ...
                pass
            crumbs = []
            for cr in driver.find_elements(By.XPATH, '//div[@class="breadcrumb_item"]//span[@property="name"]'):
                crumbs.append(cr.text)
            item_crumbs = json.dumps(crumbs)

            try:
                flag_try = 3
                WebDriverWait(driver, 2).until(lambda d: d.find_element(By.XPATH, '//div[@class="prod-main"]//span[@class="itemnamedesrc"]'))
                description = driver.find_element(By.XPATH, '//div[@class="prod-main"]//span[@class="itemnamedesrc"]')
                item_desc = description.text
            except Exception as ex:
                item_desc = ''
            item_sizes = ''
            ...
            item_images = ''
            images = driver.find_elements(By.XPATH, '//div[@id="morephotosroot"]//a[@class="bigfotolink"]')
            k = 0
...
def get_links(driver) -> list:
    ...
    while True:
        if len(items) >= IN_DATA['qty_items']:
            break
        try:
            elements = driver.find_elements(By.XPATH, '//a[@class="catalog-list-caption-link"]')
            if len(elements) == 0:
                break
            for element in elements:
                title = element.find_element(By.CLASS_NAME, 'catalog-list-title').text
                items.append(
                    {
            ...
            btn_next = driver.find_element(By.XPATH, '//div[contains(@class, "pagerules")]//div[@class="next"]//a')
            page_n += 1
            driver.get(f"{IN_DATA['target_url']}?PAGEN_1={page_n}")
        except Exception as ex:
            break
