...
def get_data(driver, items, path_results) -> bool:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            try:
                flag_try = 1
                WebDriverWait(driver, 60).until(lambda d: d.find_element(By.XPATH, '//section[@class="specifications"]'))
                prop_group = driver.find_elements(By.XPATH, '//section[@class="specifications"]//div[@class="specifications__group"]')
                props = []
                for group in prop_group:
                    try:
                        for it in group.find_elements(By.CLASS_NAME, 'specifications__item'):
                            props.append({
                                'group': group.find_element(By.CLASS_NAME, 'specifications__group-name').get_attribute('innerHTML').strip().replace('"', '').replace('{', "").replace('}', ""),
                                'title': it.find_element(By.TAG_NAME, 'dt').get_attribute('innerHTML').strip().replace('"', '').replace('{', "").replace('}', ""),
                                'value': it.find_element(By.TAG_NAME, 'dd').get_attribute('innerHTML').strip().replace('"', '').replace('{', "").replace('}', "")
                            })
                    except Exception as ex:
                        pass
            ...
            try:
                flag_try = 3
                WebDriverWait(driver, 2).until(lambda d: d.find_element(By.CLASS_NAME, 'product__main-info'))
                description = driver.find_element(By.XPATH, '//div[@class="product__main-info"]//dl')
                item_desc = description.get_attribute('innerHTML').replace('"', "'").replace('{', "").replace('}', "")
            except Exception as ex:
                item_desc = ''
            item_sizes = ''
...
def get_links(driver) -> list:
    ...
    while True:
        ...
        try:
            ...
            try:
                btn_next = driver.find_element(By.XPATH, '//button[@disabled="disabled" and @aria-label="Next page"]')
                break
            except Exception as ex:
                pass
            page_n += 1
