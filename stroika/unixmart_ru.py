...
IN_DATA = {
    'name': 'unixmart.ru',
    'host': 'https://www.unixmart.ru/',
    'target_url': 'https://www.unixmart.ru/catalog/shvejjnye-mashiny',
    'qty_items': 85,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'top'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//a[@class="pager-next"]'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.ID, 'top'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = ''
            try:
                item_brand = driver.find_element(By.XPATH, '//h3[contains(text(), "Производитель")]/parent::td/parent::tr//td[@class="bold"]').text
            except Exception as ex:
                pass
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//section[@itemprop="description"]').get_attribute('innerHTML').replace('\r', '').replace('\n', '')
            item_desc = re.sub(r'<a\s.*?>', '', item_desc).replace('</a>', '')
            images = driver.find_elements(By.XPATH, '//div[@class="card-gallery hidden-xs hidden-sm"]//div[@class="card-gallery-large"]//img')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'GoodsList'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//article[@class="index-item"]/a')
        items = []
