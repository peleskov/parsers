...
IN_DATA = {
    'name': 'sdvor.com',
    'host': 'https://www.sdvor.com/',
    'target_url': 'https://www.sdvor.com/moscow/category/teploizoljatsija-5530',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[@class="pagination"]//a[@class="next"]'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_brand = driver.find_element(By.XPATH, '//div[contains(text(), "Бренд")]/..//li[@class="value"]').get_attribute('innerHTML')
            if not item_brand:
                item_brand = IN_DATA['name']
            item_price = driver.find_element(By.XPATH, '//div[@class="product-price"]//span[@class="main"]').text
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@id="ProductDetailsTabComponent"]//div[contains(@class, "content")]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//*[contains(@class, "image-main")]/img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    try:
        ...
        out_data['target_url'] = f'{IN_DATA["target_url"]}?currentPage={n}'
