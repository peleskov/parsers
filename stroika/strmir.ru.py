...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        items_list = []
        url = f"{IN_DATA['target_url']}?SHOWALL_1=1"
        page_n = 1
        driver.get(url)
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'picbox__pic'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = driver.find_element(By.XPATH, '//*[contains(text(), "Производитель")]/following-sibling::td//span').get_attribute('innerHTML').replace('\t', '')
            if not item_brand:
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@class="props_group"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '').replace('\t', '').replace('&nbsp;', '')
            item_desc = re.sub(r"(<table(.*?)<\/table>)", "", item_desc)

            images = driver.find_elements(By.XPATH, '//div[@class="picbox__pic"]//div[contains(@class, "owl-item") and not(contains(@class, "cloned"))]//a')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'webpage'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}/'
        elements = driver.find_elements(By.XPATH, '//a[contains(@class, "catalog_item__name")]')
        items = []
