...
IN_DATA = {
    'name': 'str-online.ru',
    'host': 'https://str-online.ru/',
    'target_url': 'https://str-online.ru/catalog/krovlya/myagkaya-krovlya/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'site-wrap'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//a[contains(@class, "next_page")]'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'block-gallery'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = driver.find_element(By.XPATH, '//*[contains(text(), "Производитель")]/following-sibling::td').get_attribute('innerHTML')
            if not item_brand:
                item_brand = IN_DATA['name']
            item_price = driver.find_element(By.XPATH, '//div[@data-price]').get_attribute('data-price')
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@class="title" and contains(text(), "Описание")]/../following-sibling::div').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//div[@class="block-gallery"]//a')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'catalog-page'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}/'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "short-info")]/a')
        items = []
