...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'page-wrapper'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//ul[@class="pagination"]//li[@class="last"]'))
                next_page_btn = True
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'page-wrapper'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "catalog-section-container")]//div[@class="product-item-title"]/a')
        items = []
