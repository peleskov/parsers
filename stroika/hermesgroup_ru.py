...
IN_DATA = {
    'name': 'hermesgroup.ru',
    'host': 'https://hermesgroup.ru/',
    'target_url': 'https://hermesgroup.ru/suxie_smesi/gidroizolyaciya/',
    'qty_items': 10,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'site-container'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//ul[@class="pager"]//li[@class="next"]'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_brand = driver.find_element(By.XPATH, '//div[@itemprop="brand"]/meta').get_attribute('content')
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[contains(@class, "product-card__text")]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "product-card__main-image")]//img'))
            images = driver.find_elements(By.XPATH, '//div[contains(@class, "product-card__main-image")]//img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'product_list'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}{n}/'
        elements = driver.find_elements(By.XPATH, '//div[@id="product_list"]//a[@class="product__image"]')
        items = []
