...
IN_DATA = {
    'name': 'glavsnab.net',
    'host': 'https://glavsnab.net/',
    'target_url': 'https://glavsnab.net/instrument-i-oborudovaniye/uborka-i-klining/uborochnaya-tehnika/moyki-visokogo-davleniya.html',
    'qty_items': 10,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'page-main'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//ul[@class="pagination"]//a[@class="next"]'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'page-main'))
            ...
            item_sizes = ''
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//a[@data-target="#product-description"]'))
            driver.find_element(By.XPATH, '//a[@data-target="#product-description"]').click()
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[@data-alias="description"]//div[@class="collapse-list-content--desc"]'))
            item_desc = driver.find_element(By.XPATH, '//div[@data-alias="description"]//div[@class="collapse-list-content--desc"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//a[@class="slider-item-box"]/img'))
            images = driver.find_elements(By.XPATH, '//a[@class="slider-item-box"]/img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'page-main'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?p={n}'
        elements = driver.find_elements(By.XPATH, '//div[@class="product-card__name"]/a')
        items = []
