...
IN_DATA = {
    'name': 'gdematerial.ru',
    'host': 'https://gdematerial.ru/',
    'target_url': 'https://gdematerial.ru/catalog/gruntovki-457/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'layout__content'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//a[@class="app-pagination__link"]'))
            pagination_links = driver.find_elements(By.XPATH, '//a[@class="app-pagination__link"]')
            page_max = int(pagination_links[len(pagination_links) - 1].text)
        except exceptions.TimeoutException:
            page_max = 1
        while len(items_list) < IN_DATA['qty_items'] and url and page_max >= page_n :
            page_n += 1
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 60).until(lambda d: d.find_element(By.XPATH, '//script[@data-n-head="ssr"][@type="application/ld+json"]'))
            info = driver.find_element(By.XPATH, '//script[@data-n-head="ssr"][@type="application/ld+json"]').get_attribute('innerHTML').replace('\r', '').replace('\n', '')
            item_info = json.loads(info)
            item_title = item_info['name']
            item_brand = item_info['brand']
            item_price = item_info['offers']['price']
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = item_info['description']

            images_urls = [item_info['image']]
            k = 0
            item_images_arr = []
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'layout__content'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//a[contains(@class, "product-snippet__wrapper--link")]')
        items = []
