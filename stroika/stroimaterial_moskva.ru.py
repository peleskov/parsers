...
IN_DATA = {
    'name': 'stroimaterial-moskva.ru',
    'host': 'https://stroimaterial-moskva.ru/',
    'target_url': 'https://stroimaterial-moskva.ru/catalog/uteplitel-optom/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'header'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//a[contains(@class, "pagination__link_arrow") and starts-with(text(),">>>")]'))
            next_page_link = driver.find_element(By.XPATH, '//a[contains(@class, "pagination__link_arrow") and starts-with(text(),">>>")]')
            if next_page_link.get_attribute('href'):
                next_page_btn = True
            else:
                next_page_btn = False
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'header'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = driver.find_element(By.XPATH, '//*[contains(text(), "Бренд")]/following-sibling::p').get_attribute('innerHTML')
            if not item_brand:
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@class="product-description"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '').replace('\t', '').replace('&nbsp;', '')
            item_desc = re.sub(r"(<table(.*?)<\/table>)", "", item_desc)

            images = driver.find_elements(By.XPATH, '//div[@class="detail__image"]')
            k = 0
            images_urls = []
            for image in images:
                images_urls.append(f"{IN_DATA['host']}{image.get_attribute('href')}")
            item_images_arr = []
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'header'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}/'
        elements = driver.find_elements(By.XPATH, '//div[@class="items-list"]//a[@data-entity="image-wrapper"]')
        items = []
