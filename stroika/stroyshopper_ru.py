...
IN_DATA = {
    'name': 'stroyshopper.ru',
    'host': 'https://www.stroyshopper.ru/',
    'target_url': 'https://www.stroyshopper.ru/category/saiding/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.TAG_NAME, 'header'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "catalog-topline")]//div[contains(@class, "pagination")]//li[contains(@class, "next")]'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[@id="calculate-modal"]/following-sibling::script[@type="application/ld+json"]'))
            item_info = json.loads(driver.find_element(By.XPATH, '//div[@id="calculate-modal"]/following-sibling::script[@type="application/ld+json"]').get_attribute('innerHTML'))
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.TAG_NAME, 'header'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}page{n}.html/'
        elements = driver.find_elements(By.XPATH, '//a[@class="item-title"]')
        items = []
