...
IN_DATA = {
    'name': 'smartcomplect.com',
    'host': 'https://smartcomplect.com/',
    'target_url': 'https://smartcomplect.com/catalog/materialy_dlya_sukhogo_stroitelstva/gipsokarton/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'next-page'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'product-gallery'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = driver.find_element(By.XPATH, '//span[@itemprop="brand"]').get_attribute('innerHTML')
            if not item_brand:
            ...
            images = driver.find_elements(By.XPATH, '//div[@class="product-gallery"]//div[contains(@class, "owl-item") and not(contains(@class, "cloned"))]/img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "product-grid")]'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}/'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "product-item")]/a')
        items = []
