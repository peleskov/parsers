...
IN_DATA = {
    'name': 'a-progress.ru',
    'host': 'https://a-progress.ru/',
    'target_url': 'https://a-progress.ru/catalog/stroymaterialy/stroitelnye_bloki_i_kirpich/',
    'qty_items': 50,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'body'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[@id="paginator"]//a[@class="right arrow"]'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.ID, 'content'))
            ...
            item_price = driver.find_element(By.XPATH, '//span[contains(@'
                                                       'class, "price")]').text
            item_price = re.sub(r"[^\d\.]", "", item_price)
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@id="detailtext"]//div[@class="contentinner"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[@class="fancygallery"]//td[@class="image"]/img'))
            images = driver.find_elements(By.XPATH, '//div[@class="fancygallery"]//td[@class="image"]/img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'prods'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}'
        elements = driver.find_elements(By.XPATH, '//div[@class="padd"]/a')
        items = []
