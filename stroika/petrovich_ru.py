...
IN_DATA = {
    ...
    'target_url': 'https://petrovich.ru/catalog/1557/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'main_container'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//button[@data-test="last-page-button"]'))
            last_page_btn = True
        except exceptions.TimeoutException:
            last_page_btn = False
        while len(items_list) < IN_DATA['qty_items'] and url and last_page_btn:
            page_n += 1
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.CLASS_NAME, 'product-body'))
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@data-test="product-description"]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[@class="content-slide"]/img'))
            images = driver.find_elements(By.XPATH, '//div[@class="content-slide"]/img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'main_container'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?p={n}'
        elements = driver.find_elements(By.XPATH, '//a[@data-test="product-image"]')
        items = []
