...
IN_DATA = {
    ...
    'target_url': 'https://www.ekonomstroy.ru/catalog/instrument/',
    'qty_items': 1000,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'bx_eshop_wrap'))
        ...
        while len(items_list) < IN_DATA['qty_items'] and url and next_page_btn:
            ...
            url = data['target_url']
            try:
                WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[@class="navigation_api nav_top"]//div[@class="bx_pagination_page"]//li[last()]/a'))
                next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[@itemprop="description"]').get_attribute('innerHTML').replace('\r', '').replace('\n', '')
            item_desc = re.sub(r'<a\s.*?>', '', item_desc).replace('</a>', '')
            images = driver.find_elements(By.XPATH, '//div[@class="bx_bigimages_aligner"]//img')
            k = 0
            item_images_arr = []
            for image in images:
                k += 1
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.CLASS_NAME, 'bx_catalog_item_images'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_2={n}'
        elements = driver.find_elements(By.CLASS_NAME, 'bx_catalog_item_images')
        items = []
