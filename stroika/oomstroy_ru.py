...
IN_DATA = {
    'name': 'oomstroy.ru',
    'host': 'https://oomstroy.ru/',
    'target_url': 'https://oomstroy.ru/uteplitel-zvukoizoljacija/mineralnaja-vata/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'main-content'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//ul[@class="pagination"]//li'))
            pagination_last = driver.find_element(By.XPATH, '//ul[@class="pagination"]//li[last()]')
            if pagination_last.get_attribute('class') != 'active':
                next_page_btn = True
            else:
                next_page_btn = False
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            item_brand = driver.find_element(By.XPATH, '//span[@itemprop="brand"]').text
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.ID, 'tab-description').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//div[@id="imageWrap"]//img')
            k = 0
            images_urls = []
            for image in images:
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.ID, 'content'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?page={n}'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "products_category")]//link')
        items = []
