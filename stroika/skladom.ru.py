...
IN_DATA = {
    'name': 'skladom.ru',
    'host': 'https://skladom.ru/',
    'target_url': 'https://skladom.ru/katalog/stroymaterialy/sypuchie-materialy/shtukaturnye-smesi/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.ID, 'tygh_container'))
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[@class="ty-pagination__bottom"]//a[contains(@class, "ty-pagination__right-arrow")]'))
            next_page_link = driver.find_element(By.XPATH, '//div[@class="ty-pagination__bottom"]//a[contains(@class, "ty-pagination__right-arrow")]')
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "product_block")]'))
            ...
            item_sizes = ''
            item_desc = driver.find_element(By.ID, 'content_description').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')

            images = driver.find_elements(By.XPATH, '//div[contains(@class, "ty-product-img")]//img')
            k = 0
            images_urls = []
            for image in images:
                images_urls.append(image.get_attribute('data-cloudzoom'))
            item_images_arr = []
            for item_image_url in set(images_urls):
                k += 1
                item_image_url = item_image_url.replace('zoomImage: ', '').replace('"', '')
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "main-content-grid")]'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}page-{n}/'
        elements = driver.find_elements(By.XPATH, '//div[@class="grid-list"]//a[@class="product-title"]')
        items = []
