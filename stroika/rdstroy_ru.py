...
IN_DATA = {
    'name': 'rdstroy.ru',
    'host': 'https://rdstroy.ru/',
    'target_url': 'https://rdstroy.ru/catalog/bitumnye_materialy/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        driver.get(url)
        try:
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.XPATH, '//div[@class="bx_pagination_page"]//li[last()]/a'))
            next_page_btn = True
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.ID, 'plaice_holder'))
            ...
            item_price = driver.find_element(By.XPATH, '//span[@itemprop="lowPrice"]').get_attribute('data-price_e')
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "catalog-elements")]'))
        out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}&SIZEN_1=29'
        elements = driver.find_elements(By.XPATH, '//div[contains(@class, "catalog-elements")]//a[@itemprop="name"]')
        items = []
