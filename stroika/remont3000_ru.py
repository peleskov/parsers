...
IN_DATA = {
    ...
    'target_url': 'https://www.remont3000.ru/catalog/obshchestroitelnye_smesi/',
    'qty_items': 1,
}
...
def get_items():
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        ...
        try:
            ...
            WebDriverWait(driver, 10).until(lambda d: d.find_element(By.CLASS_NAME, 'mainpage'))
...
def get_data(driver, items) -> list:
    ...
    for item in items[:IN_DATA['qty_items']]:
        try:
            ...
            driver.get(item[0])
            WebDriverWait(driver, 20).until(lambda d: d.find_element(By.XPATH, '//span[@itemprop="price"]'))
            item_title = driver.find_element(By.TAG_NAME, 'h1').text
            item_brand = driver.find_element(By.XPATH, '//div[@itemprop="brand"]').text
            # item_brand = IN_DATA['name']
            item_price = driver.find_element(By.XPATH, '//span[@itemprop="price"]').get_attribute('innerHTML')
            item_id = hashlib.sha256(f"{item_title}{item_brand}{item_price}{item[0]}".encode("utf-8")).hexdigest()
            item_sizes = ''
            item_desc = driver.find_element(By.XPATH, '//div[contains(@class, "text_desc")]').get_attribute('innerHTML') \
                .replace('\r', '').replace('\n', '')
...
def get_links(driver, page_url, n) -> dict:
    ...
    driver.get(page_url)
    try:
        WebDriverWait(driver, 30).until(lambda d: d.find_element(By.XPATH, '//div[contains(@class, "catalog_i_list")]'))
        # out_data['target_url'] = f'{IN_DATA["target_url"]}?PAGEN_1={n}&SIZEN_1=29'
        elements = driver.find_elements(By.XPATH, '//a[contains(@class, "link_block")]')
        items = []
